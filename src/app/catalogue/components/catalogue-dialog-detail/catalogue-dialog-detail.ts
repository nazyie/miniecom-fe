...
@Component({
  ...
  imports: [ReactiveFormsModule, InventoryStockTable, CatalogueVariantDialog],
  templateUrl: './catalogue-dialog-detail.html',
  styleUrl: './catalogue-dialog-detail.css'
})
export class CatalogueDialogDetail implements OnInit, AfterViewInit{
  ...
  modalAttachmentIsOpen: boolean = false;
  ...
  handleClosingDialogAttachment(action: boolean) {
    this.modalAttachmentIsOpen = action;
  }
  ...
  openModalAttachment() {
    ...
    if (this.formMode === 'CREATE' && this.form.controls['id'].value == '') {
      if (this.form.valid) {
        ...
        this.modalAttachmentIsOpen = true;
      }
    } else {
        this.modalAttachmentIsOpen = true;
    }
